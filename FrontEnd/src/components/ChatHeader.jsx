...
const ChatHeader = ()=> {
  const {selectedUser,setSelectedUser,deleteAllMsgs} = useChatStore()
  const {onlineUsers,authUser} = useAuthStore()
  // console.log("ChatHeader :",selectedUser)
  const handleDeleteMany = (receiverId)=>{
         deleteAllMsgs(receiverId);
  }
  return (
    <div className="min-h-[70px] h-[10vh] bg-black flex items-center justify-between">
         ...
         <span className="flex gap-2">
          <button onClick={()=>{handleDeleteMany(selectedUser?._id)}}className="p-1 rounded-md bg-slate-500 flex justify-center items-center hover:bg-red-800"><Trash size={24}/>Clear Chat</button>
